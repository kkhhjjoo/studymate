...
export default function EditStudyPage({ params }: EditStudyPageProps) {
  ...
  useEffect(() => {
    ...
    setSelectedTags(study.tags);
    setMaxMembers(study.maxMembers.toString());
    setSchedule(study.schedule);
    setStartDate(study.startDate);
    setEndDate(study.endDate ?? '');
    const locIdx = LOCATIONS.findIndex((l) => l.name === study.location.name);
  ...
  if (!currentUser || study.hostId !== currentUser._id) {
    return (
      <div className="min-h-screen bg-background">
        <Header />
        <main className="container mx-auto px-4 py-16 text-center">
  ...
  const handleSubmit = (e: { preventDefault: () => void }) => {
    e.preventDefault();
    if (!title || !description || !category || !locationIndex || !schedule || !startDate) return;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">스터디 수정</CardTitle>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {selectedTags.map((tag) => (
                    <Badge key={tag} variant="default" className="gap-1">
                      {tag}
                      <button type="button" onClick={() => toggleTag(tag)} className="ml-1 hover:bg-primary-foreground/20 rounded-full">
                        <X className="h-3 w-3" />
                      </button>
                    </Badge>
                  ))}
                ...
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {POPULAR_TAGS.filter((tag) => !selectedTags.includes(tag)).slice(0, 10).map((tag) => (
                    <Badge key={tag} variant="outline" className="cursor-pointer hover:bg-secondary" onClick={() => toggleTag(tag)}>
                      {tag}
                    </Badge>
                  ))}
                </div>
